...
export class App {
  ...
  updateLiveQuantity(event: { id: number; quantity: number }) {
    this.cart.update((currentCart) => {
      // Kiểm tra xem sản phẩm này đã được "Buy Now" (có trong giỏ) chưa
      const isExisted = currentCart.some((item) => item.id === event.id);

      if (isExisted) {
        // Nếu đã có trong giỏ, cập nhật số lượng để totalAmount tự nhảy
        return currentCart.map((item) =>
          item.id === event.id ? { ...item, quantity: event.quantity } : item,
        );
      }
